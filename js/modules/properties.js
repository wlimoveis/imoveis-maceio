// js/modules/properties.js - VERSÃO ATUALIZADA (loadPropertyList movida para Support System)
console.log('✅ properties.js carregado - loadPropertyList movida para Support System');
...
window.addNewProperty = async function(propertyData) {
    ...
    try {
        ...
        // loadPropertyList agora é fornecido pelo Support System (admin-list-ui.js)
...
window.updateLocalProperty = function(propertyId, updatedData) {
    ...
    window.savePropertiesToStorage();
    // loadPropertyList agora é fornecido pelo Support System
    if (typeof window.loadPropertyList === 'function') setTimeout(() => window.loadPropertyList(), 100);
...
// ========== FALLBACK PARA loadPropertyList (apenas se Support não carregar) ==========
if (typeof window.loadPropertyList !== 'function') {
    window.loadPropertyList = function(page) {
        console.log('ℹ️ [Core] loadPropertyList: UI completa disponível apenas com ?debug=true');
        const container = document.getElementById('propertyList');
        if (container) {
            const countElement = document.getElementById('propertyCount');
            const totalItems = window.properties?.length || 0;
            ...
            container.innerHTML = `
                <div style="background: #e8f4fd; padding: 1rem; border-radius: 8px; text-align: center;">
                    <i class="fas fa-chart-line" style="font-size: 2rem; color: var(--primary); margin-bottom: 0.5rem; display: block;"></i>
                    <p><strong>📊 Painel do Corretor</strong></p>
                    <p>Total de imóveis: <strong>${totalItems}</strong></p>
                    <hr style="margin: 1rem 0;">
                    <p style="font-size: 0.85rem; color: #666;">
                        <i class="fas fa-info-circle"></i> Para funcionalidades completas (estatísticas, paginação, edição avançada),
                        <br>acesse com <code>?debug=true</code> ou aguarde o carregamento do módulo.
                    </p>
                </div>
            `;
        }
