...
(function fixMediaSystemLoad() {
    ...
    const originalLoadExisting = window.MediaSystem?.loadExisting;
    
    if (!originalLoadExisting) {
        console.error('❌ [FIX] MediaSystem.loadExisting não encontrado!');
        return;
    }
    
    // Substituir por versão corrigida
    window.MediaSystem.loadExisting = function(property) {
        console.log('🚨 [FIX] loadExisting CORRIGIDO sendo executado');
        console.log('📦 property recebido:', property);
        
        // FORÇAR PROCESSAMENTO CORRETO DOS DADOS
        const fixedProperty = {
            ...property,
            // Garantir que images é uma string processável
            images: (() => {
                if (!property.images || property.images === 'EMPTY' || property.images === '') {
                    console.log('⚠️ [FIX] property.images vazio, tentando buscar...');
                    
                    // Tentar buscar do objeto original
                    if (property.image_urls && Array.isArray(property.image_urls)) {
                        return property.image_urls.join(',');
                    }
                    if (property.media && Array.isArray(property.media)) {
                        return property.media.join(',');
                    }
                    if (property.fotos && Array.isArray(property.fotos)) {
                        return property.fotos.join(',');
                    }
                    return '';
                }
                ...
                if (Array.isArray(property.images)) {
                    return property.images.filter(url => url && url !== 'EMPTY').join(',');
                }
                
                return property.images;
            })(),
            
            // Garantir que pdfs é uma string processável
            pdfs: (() => {
                if (!property.pdfs || property.pdfs === 'EMPTY' || property.pdfs === '') {
                    console.log('⚠️ [FIX] property.pdfs vazio, tentando buscar...');
                    
                    // Tentar buscar do objeto original
                    if (property.pdf_urls && Array.isArray(property.pdf_urls)) {
                        return property.pdf_urls.join(',');
                    }
                    if (property.documentos && Array.isArray(property.documentos)) {
                        return property.documentos.join(',');
                    }
                    return '';
                }
                
                // Se for array, converter para string
                if (Array.isArray(property.pdfs)) {
                    return property.pdfs.filter(url => url && url !== 'EMPTY').join(',');
                }
        ...
        console.log('📊 [FIX] Dados processados:');
        console.log('  images:', fixedProperty.images?.substring(0, 100));
        console.log('  pdfs:', fixedProperty.pdfs?.substring(0, 100));
...
if (originalEditProperty) {
    window.editProperty = function(id) {
        console.log('🔧 [FIX] editProperty chamado para ID:', id);
        
        const property = window.properties?.find(p => p.id === id);
        if (!property) {
            console.error('❌ [FIX] Imóvel não encontrado:', id);
            return originalEditProperty(id);
        }
        
        // Log dos dados originais
        console.log('📊 [FIX] Dados originais do imóvel:');
        console.log('  images:', property.images);
        console.log('  images type:', typeof property.images);
        console.log('  pdfs:', property.pdfs);
        console.log('  pdfs type:', typeof property.pdfs);
        
        // Chamar função original
        const result = originalEditProperty(id);
        
        // FORÇAR recarregamento da mídia após um delay (caso o loadExisting falhe)
        setTimeout(() => {
            if (window.MediaSystem) {
                console.log('🔄 [FIX] Forçando recarregamento da mídia...');
                window.MediaSystem.loadExisting(property);
            }
        }, 500);
    ...
    console.log('✅ [FIX] editProperty interceptado com sucesso!');
}
